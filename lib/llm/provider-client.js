...
async function requestJsonWithCurlFallback(endpoint, { method = "POST", headers = {}, body = null, timeoutMs = 60000, signal } = {}) {
  ...
  try {
    ...
    const args = ["-sS", "--max-time", String(Math.ceil(timeoutMs / 1000)), "-X", method, endpoint];
    ...
    const rawText = m ? txt.slice(0, m.index).trim() : txt;
    return { ok: status >= 200 && status < 300, status, rawText };
  }
}
...
export async function callProviderText({
  ...
  const res = await requestJsonWithCurlFallback(endpoint, {
    method: "POST",
    headers,
    body,
    timeoutMs,
