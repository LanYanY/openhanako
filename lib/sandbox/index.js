...
function createFullAccessToolset(cwd, customTools, readOps) {
...
export function createSandboxedTools(cwd, customTools, { agentDir, workspace, hanakoHome, mode }) {
  ...
  if (policy.mode === "full-access") {
    return createFullAccessToolset(cwd, customTools, readOps);
  }
  ...
  if (!checkAvailability(platform)) {
    console.warn(`[sandbox] ${t("sandbox.osRequired", { platform })}，已自动降级到 full-access。`);
    return createFullAccessToolset(cwd, customTools, readOps);
  }
